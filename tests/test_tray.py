...
def test_tray_manager_creates_menu_items():
    ...
    mgr = TrayManager(on_start=MagicMock(), on_stop=MagicMock(), on_quit=MagicMock())
    ...
    assert len(raw_items) == 4  # Start, Stop, SEPARATOR, Quit


def test_tray_manager_calls_start_callback():
    from tray import TrayManager
    start_cb = MagicMock()
    mgr = TrayManager(on_start=start_cb, on_stop=MagicMock(), on_quit=MagicMock())
...
def test_tray_manager_calls_stop_callback():
    from tray import TrayManager
    stop_cb = MagicMock()
    mgr = TrayManager(on_start=MagicMock(), on_stop=stop_cb, on_quit=MagicMock())
